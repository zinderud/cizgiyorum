...
export class HeadComponent implements OnInit, AfterContentInit {
  ...
  constructor(
    ...
    this.complexForm = _formBuilder.group({
      ...
      'pass': [null, Validators.compose([Validators.required, Validators.minLength(3)])]
    });
  }
  ...
  ngAfterContentInit() {
     if (this._router.url === '/cizimler#search') {
  ...
  public seachFocus() {
    if (this._router.url !== '/portofolios') {
      ...
      this.searchCizimler(this.searchInfo);
      this._router.navigate(['/cizimler'], {fragment: 'search'});
    }
  ...
  public seachBlur() {
    ...
    return this._router.navigate(['/cizimler']);
  }
